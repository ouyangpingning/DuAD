#!/usr/bin/env python3
"""汇总 SimpleNet model_log/simplenet/ 下所有 *_full.log 的最终评估指标，按类别统计均值 ± 标准差。

结果自动保存至 results/simplenet/ 目录（CSV 文件），同时打印到终端。

用法:
    python simplenet/aggregate_results.py                        # 默认 model_log/simplenet/
    python simplenet/aggregate_results.py /path/to/logs          # 指定日志目录
    python simplenet/aggregate_results.py --csv                  # stdout 输出 CSV（不保存文件）
"""
...
def extract_dataset(log_path: Path, log_dir: Path) -> str:
    ...
    例如: model_log/simplenet/mvtec/bottle/bottle_full.log → "mvtec"
          model_log/simplenet/visa/candle/candle_full.log  → "visa"
    """
    try:
        ...
        for key, label in METRIC_NAMES:
            ...
            for ds, cat_means in sorted(ds_means.items()):
                if cat_means:
                    ...
                    std = var ** 0.5
                    ds_label = f"{label} [{ds}]" if ds else label
                    print(f"  {ds_label:<16} {overall:>8.4f} ± {std:.4f}")
