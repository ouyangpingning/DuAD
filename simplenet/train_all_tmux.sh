#!/bin/bash
# SimpleNet baseline 批量训练脚本 — tmux 多会话并行
# 用法: bash simplenet/train_all_tmux.sh
...
# ==================== 交互式配置 ====================
echo "================================================"
echo "  SimpleNet Baseline — 批量训练脚本"
echo "  WideResNet50-2 | CVPR 2023"
echo "================================================"
echo ""
...
while [ $task_cursor -lt $total_tasks ]; do
    ...
    if [ "$mode_choice" == "2" ]; then
        ...
        session_name="sn_k${k_shot}_s${current_seed}_g${session_num}"
    else
        cmd_args="--categories \"${session_cats}\""
        session_name="sn_g${session_num}"
    fi
    ...
    tmux send-keys -t "$session_name" "python simplenet/main.py ${cmd_args}" C-m
    sleep 2
...
echo ""
echo "会话命名规则 (sn = SimpleNet):"
echo "  全样本:   sn_g{N}"
echo "  少样本:   sn_k{K}_s{seed}_g{N}"
echo ""
echo "使用以下命令管理会话:"
