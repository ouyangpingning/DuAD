...
class PCAMaskGenerator:
    ...
    def _compute_first_pc_torch(self, features: torch.Tensor) -> torch.Tensor:
        """
        计算第一主成分投影值（GPU/CPU自动选择）
        
        使用PyTorch SVD实现，可在GPU上运行
        
        Args:
            features: [N, C] torch张量
        Returns: [N] 第一主成分投影值
        """
        # 中心化
    ...
    def fit(self, train_dataloader) -> Dict[str, float]:
        ...
        if self.trainer is None:
            self.trainer = Trainer(
                ...
                self.projection, # 投影器
                self.discriminator, # 判别器
                self.config, # 模型配置类
                self.logger # 日志
            )
            # 确保PCA掩模生成器知道当前类别（如果有）
    ...
    def evaluate(
        ...
        if not compute_full_metrics:
            if len(masks_gt) > 0:
                ...
                seg_norm = np.zeros_like(seg_arr, dtype=np.float32)
                for min_val, max_val in zip(seg_mins, seg_maxs):
                    normalized = (seg_arr - min_val) / max(max_val - min_val, 1e-2)
                    seg_norm += normalized
                seg_norm = seg_norm / len(segmentations) # 平均归一化结果
        ...
        if len(masks_gt) > 0:
            ...
            for min_val, max_val in zip(seg_mins, seg_maxs):
                # 用当前样本的 min/max 归一化所有图像
                normalized = (seg_arr - min_val) / max(max_val - min_val, 1e-2)
                seg_norm += normalized
            
            # 求平均（除以样本数）
            seg_norm = seg_norm / len(segmentations)
