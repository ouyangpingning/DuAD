...
主 checkpoint 中嵌入 PCA Student 状态：

- **保存**: `save()` → `state['pca_student_state']` + `state['pca_student_config']`
- **恢复**: `load()` → 重建 PCAStudent → 挂接到 Trainer 的 pca_generator
- **跳过重训**: `train_pca_student()` 检测 `self.pca_student is not None` → 直接返回
...
| `main.py` `train_category()` | `src/main.py` | 编排：load → 文件锁 → train → save，含断点续训 |
