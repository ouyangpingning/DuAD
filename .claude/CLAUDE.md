...
python src/viz/visualize_feature.py --categories "bottle screw" --k_shot 4 --shot_seed 0
python src/viz/visualize_feature.py --categories "bottle screw" --num_samples 8
python src/viz/visualize_feature.py --categories "bottle" --skip_inference  # analysis-only, no .pth needed
...
python src/deploy/export_onnx.py --category bottle --k_shot 2 --shot_seed 0 --verify
...
# Or if already cloned: git submodule init && git submodule update
...
Unsupervised anomaly detection on MVTec AD using frozen DINOv2 (`dinov2_vits14_reg`) features + dual-branch GAN-style discriminator. Extracts multi-layer features at indices `[2,5,8,11]`, aggregates them via `FeatureAggregator` (default: 3×3 neighborhood Unfold + cross-layer AdaptiveAvgPool1d), then trains a 2-layer MLP projection + 2-layer MLP discriminator (Hinge/BCE loss) to distinguish real normal features from noise-corrupted fake features. At inference, the negative discriminator output is the anomaly score.

## Key files

| File | Role |
|------|------|
| `src/myAD.py` | Core: `ModelConfig`, `FeatureAggregator`, `FeatureExtractor`, `PCAMaskGenerator`, `PerlinMaskGenerator`, `PCAStudent`, `Projection`, `Discriminator`, `Trainer`, `Predictor`, `DINOv2AnomalyDetector` |
...
| `src/utils.py` | Metrics (AUROC, AP, F1, PRO), `_embed_legacy` (deprecated, use `FeatureAggregator`), logger setup, DINOv2 loader |
| `src/myAD.py` (PerlinMaskGenerator) | Perlin noise mask generation (absorbed from former `src/perlin.py`) |
| `src/viz/visualize_feature.py` | Inference/visualization entry point. Uses `CategoryVisualizer` class (Strategy + Template Method) to orchestrate 5 visualization types: anomaly heatmaps (random sampling, percentile norm, F1 threshold, plasma colormap, background NaN), PCA masks (SVD vs PCA Student), Perlin masks, DINOv2 feature maps, and data augmentation previews. Supports `--num_samples`, `--skip_inference` |
| `src/deploy/export_onnx.py` | ONNX model export for deployment inference (no PyTorch dependency needed) |
...
**Training**: `src/main.py` → `DINOv2AnomalyDetector.fit()` → `Trainer.train_epoch()`:
  ...
  2. Aggregate via `FeatureAggregator` (default "neighborhood": patchify → adaptive pool per layer → concatenate → pool to 1536-d; or "channel_concat": simple channel-wise concat)
...
- **All hyperparameters are managed in `config.toml`** — edit this file to change params; both `main.py` and `src/viz/visualize_feature.py` read from here
- `cv2`, `pandas`, `skimage` are imported directly in the files that need them (not via `commen_import`)
- DINOv2 is loaded via `torch.hub.load` with `source='local'` from `facebookresearch_dinov2_main/` (git submodule — clone with `--recurse-submodules`)
...
- Before making changes: 'Read the current eval code and explain exactly what F1 calculation it performs. Confirm with me whether any change is actually needed before editing.'
- This project runs on Arch Linux (local) and Ubuntu (server via SSH at 10.20.9.202 -p 20319). GPU is available on the server. When editing server files, use sshfs or direct SSH. Python environment uses conda.
## 行为准则
...
- Before any git push or pull, verify the proxy is running (127.0.0.1:7890) and check for remote force-pushes with `git fetch --dry-run` first. If branch has diverged, prefer `git reset --hard origin/<branch>` over rebase for force-pushed remotes.